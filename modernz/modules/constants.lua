-- modernz :: modules/constants.lua
-- Shared constants and asset data (mpv logo, Santa hat).
...
-- Color option defaults — single source of truth. options.lua copies these
-- into user_opts (so the runtime table stays user-overridable), and
-- config.lua validates user values against them.
...
-- Default speed presets for the speed menu, used as a fallback when the
-- user_opts.speed_presets string parses to nothing.
local DEFAULT_SPEED_PRESETS = {0.5, 0.75, 1.0, 1.25, 1.5, 2.0}
...
return {
    ...
    santa_hat_lines = santa_hat_lines,
    COLOR_DEFAULTS = COLOR_DEFAULTS,
    DEFAULT_SPEED_PRESETS = DEFAULT_SPEED_PRESETS,
}
